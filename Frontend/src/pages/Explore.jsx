...
// const stripePromise = loadStripe('pk_test_51RYl0pE2bSD5hjDAynAHttnRrEhIhlUBWqNzvg6dauSUPrxXLEWMyBfa8pLYhssuSuLqBd7347LJONXCG7GdfkEr005fZBJRmM');
...
//                                 <StripeCheckoutForm
//                                   amount={painting.prices * 100} // Stripe expects amount in cents
//                                   userId={userId}
//                                   items={()=>({
//                                     itemproductId: painting.id,
//                                     price: parseFloat(painting.prices),
//                                     quantity: 1
//                                   })}
//                                   onSuccess={handleBuyNow(painting.id)}
//                                   onError={handleError}
//                                 />
//                               </Elements>
//                             </div>
...
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { FaSearch, FaExclamationCircle, FaPalette, FaShoppingCart, FaCheckCircle, FaTimes } from 'react-icons/fa';
import { loadStripe } from '@stripe/stripe-js';
import StripeCheckoutForm from '../components/StripeCheckoutForm';
import { Elements } from '@stripe/react-stripe-js';
...
const Explore = () => {
  ...
  const [cartStatus, setCartStatus] = useState({ id: null, loading: false, success: false });
  const [purchaseStatus, setPurchaseStatus] = useState({ id: null, loading: false });
  const [showStripe, setShowStripe] = useState({ show: false, paintingId: null });
  const [userId, setUserId] = useState(null);
  ...
  const handleBuyNowClick = (paintingId) => {
    if (userId === null) {
      navigate('/auth');
      return;
    }
    setShowStripe({ show: true, paintingId });
  };

  const handlePaymentSuccess = async (paintingId) => {
      setPurchaseStatus({ id: paintingId, loading: true });
      try {
        await axios.post(`http://localhost:8090/api/v1/painting/${userId}/buyitem/${paintingId}`);
        setPurchaseStatus({ id: null, loading: false });
        setPaintings(paintings.filter(p => p.id !== paintingId));
        setShowStripe({ show: false, paintingId: null });
        navigate('/', { 
          state: { 
            message: 'Purchase successful! Thank you for your order.',
            type: 'success'
  ...
  const currentPainting = paintings.find(p => p.id === showStripe.paintingId);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {showStripe.show && currentPainting && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="relative max-w-md w-full mx-4">
              ...
              <Elements stripe={stripePromise}>
                <StripeCheckoutForm
                  amount={currentPainting.prices * 100} // Stripe expects amount in cents
                  userId={userId}
                   items={[
                        {
                          productId: currentPainting.id,
                          price: parseFloat(currentPainting.prices),
                          quantity: 1,
                        },
                      ]}
                  onSuccess={() => handlePaymentSuccess(currentPainting.id)}
                  onError={handleError}
                />
              </Elements>
            </div>
                      ...
                      <div className="flex space-x-2">
                        {cartStatus.id === painting.id ? (
                          ...
                          <>
                            ...
                            <button
                              onClick={() => handleBuyNowClick(painting.id)}
                              disabled={purchaseStatus.id === painting.id}
                              className={`flex items-center space-x-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors ${
                                purchaseStatus.id === painting.id ? 'opacity-75 cursor-not-allowed' : ''
                              }`}
